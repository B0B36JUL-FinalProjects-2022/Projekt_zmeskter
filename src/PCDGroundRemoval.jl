...
function make_pillar_grid(points::Matrix{Float64}; grid_box_size = 1.8)
    """
    ...
    for i = 0:x_size-1
        for j =0:y_size-1 
            ...
            x_test = findall(in(x_test1),x_test2)
            y_test = findall(in(y_test1),y_test2)
...
function my_extremas(points::Matrix{Float64}, grid_box_size)
